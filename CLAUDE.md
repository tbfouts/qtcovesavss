...
A KUKSA Databroker backend plugin (`src/plugins/kuksa/`) connects to Eclipse KUKSA via gRPC to provide live VSS data. It is conditionally built when Qt Protobuf and Grpc modules are available. Backend plugins live under `src/plugins/` to separate them from the frontend modules.
...
Located in `src/plugins/kuksa/`. A single Qt IF backend plugin that connects all 36 interfaces to a KUKSA Databroker via gRPC.
...
Proto files are vendored from `eclipse-kuksa/kuksa-databroker` in `src/plugins/kuksa/proto/kuksa/val/v2/`. The import in `val.proto` was changed to `import "types.proto"` (relative) because Qt's `qt_add_protobuf` auto-adds the proto file directory as a protoc include path.
